...
이게 왜 DP일까 고민하다 결국 풀이 봤음.. 근데 알고보니 A전봇대 기준 정렬하고 나면 B에서 ```가장 긴 증가하는 수열```구하기만 하면 됐다. 충격받음.. <br>
내가 생각한 조건인 A값이 크면 B값이 작고 or A값이 작은면 B값이 큰 것들을 찾는 과정이 ```가장 긴 증가하는 수열``` 찾는 거로 해결되는 거였다.
